...
load_model_dir = 'Pleural_effusion/Global_good/Fully_global/ppnet_chest/'
load_model_name = 'client_3_final_round_2_0.7381.pth'
load_model_path = os.path.join(load_model_dir, load_model_name)
client_num = 3 # specify client number

epoch_number_str = str(2) # for GM and PM set the epoch number manually
...
load_img_dir = 'Pleural_effusion/Global_good/Fully_global/prot_chest/' # directory with saved prototypes

prototype_info = np.load(os.path.join(load_img_dir, 'epoch-'+epoch_number_str, 'client_' + str(client_num) + '_prot_bb'+epoch_number_str+'.npy')) # set client number
...
def save_prototype(fname, epoch, index):
    p_img = plt.imread(os.path.join(load_img_dir, 'epoch-'+str(epoch), 'client_' + str(client_num)+ '_prot_img'+str(index)+'.png')) # set client number
    #plt.axis('off')
...
def save_prototype_self_activation(fname, epoch, index):
    p_img = plt.imread(os.path.join(load_img_dir, 'epoch-'+str(epoch),
                                    'client_' + str(client_num) + '_prot_img-original_with_self_act'+str(index)+'.png')) # set client number
    #plt.axis('off')
    plt.imsave(fname, p_img)
...
def save_prototype_original_img_with_bbox(fname, epoch, index,
    ...
    p_img_bgr = cv2.imread(os.path.join(load_img_dir, 'epoch-'+str(epoch), 'client_' + str(client_num) + '_prot_img-original'+str(index)+'.png')) # set client number
